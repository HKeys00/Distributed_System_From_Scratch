    ...
    public class Worker : BackgroundService
    {
        ...
        private async Task OnProcessOutboxQueue()
        {
            ...
            try
            {
                ...
                var oldestScheduled = await context.Outbox.Where(t => t.NextAttemptAt > DateTime.UtcNow).MinAsync(t => (DateTime?)t.CreatedAt);
                var oldestUnpublished = await context.Outbox.Where(t => t.NextAttemptAt < DateTime.UtcNow).MinAsync(t => (DateTime?)t.NextAttemptAt);
